class B
  ...
  def self.test
    url = 'http://ae.cnmsl.net//MachineData/20071113145629.htm'
    info(url)
  end

  def self.info(url)
    response = Typhoeus::Request.get("#{url}")
    info_trs = Nokogiri::HTML(response.body).css('table')[4].css('tr')# rescue []
    var = {}
    var['summary'] = info_trs[1]
